...
pair<vector<Solution>, GaRunMetrics> GA::run(const Instance &inst)
{
    ...
    {
        GaGeneration genMetrics;
        genMetrics.generation_index = gen;
        Timer gen_timer;
        gen_timer.start();
        ...
        nextGeneration(genMetrics, params_.use_local_search, current_mutation_rate);

        gen_timer.stop();
        genMetrics.time_total_ms = gen_timer.elapsedMs();
        genMetrics.best_cost = population[0].total_cost;
        ...
        {
            sum_cost += individual.total_cost;
            sum_open += individual.num_open_facilities;
        }
        genMetrics.avg_cost = sum_cost / population.size();
        genMetrics.avg_open_facilities = (double)sum_open / population.size();

        // Calculate Diversity (Standard Deviation of Costs)
        double sum_sq_diff = 0.0;
        for (const auto &individual : population)
        {
            double diff = individual.total_cost - genMetrics.avg_cost;
            sum_sq_diff += diff * diff;
        }
        genMetrics.cost_std_dev = sqrt(sum_sq_diff / population.size());

        // Calculate Uniqueness Ratio
        genMetrics.unique_ratio = 1.0 - ((double)genMetrics.duplicates / (double)params_.pop_size);
        if (genMetrics.unique_ratio < 0)
            genMetrics.unique_ratio = 0;

        run_data.history.push_back(genMetrics);

        // --- STAGNATION CHECK ---
        if (genMetrics.best_cost < global_best_cost)
        {
            global_best_cost = genMetrics.best_cost;
...
void GA::nextGeneration(GaGeneration &current_metrics, bool use_local_search, double current_mutation_rate)
{
    ...
    for (int i = 0; i < needed; ++i)
    {
        ...
        while (isDuplicate(childreen[i]) && attempts < 3)
        {
            ...
            int idx = distN(rng);
            bool was_open = childreen[i].openFacilities[idx];
            childreen[i].openFacilities[idx] = !was_open;
            if (was_open)
                childreen[i].num_open_facilities--;
            else
                childreen[i].num_open_facilities++;
...
vector<Solution> GA::selectElite(int k) const
{
    ...
    for (int i = 0; i < min((int)population.size(), k); ++i)
